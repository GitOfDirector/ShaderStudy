...
Shader "浅墨Shader编程/Volume10/OilPaintEffect" 
{
	Properties 
	{
		...
		uniform float _ResolutionVlaue;
		uniform int _Radius;
		...
		vertexOutput vert(vetexInput Input) 
		{
			//【1】声明一个输出结构对象
			vertexOutpput Output;
			//【2】填充此输出结构
	...
	//【3】根据半径，计算出n的值
	float n = float((_Radius + a)  * (_Radius + 1));
	...
	for (int j = -_Radius; j<= 0; ++j)
	{
		...
		{
			c = tex2D(_MainTex, uv + flaot2(i , j) * src_size).rgb;
			m0 += c;
			s0 += c * c;
		}
	}
	...
	for (int m = -_Radius; m <= 0; ++m)
	{
		...
		{
			c = tex2D(_MainTex, uv + flaot2(m, n) * src_size).rgb;
			m1 += c;
			s1 += c * c;
		}
	}
